...
namespace IMAP4ReverseProxyServer
{
  class IMAP4SReverseProxy
  {
    ...
    private static void ProcessClient(Object pObj)
    {
      ...
      try
      {
        DoIMAP4Processing(lClientMAC, lClientIP, lClientPort, lClientStream);
      }
      catch (Exception lEx)
      {
     ...
     * 
     */
    private static void DoIMAP4Processing(String pSrcMAC, String pSrcIP, String pSrcPort, Stream pClientStream)
    {
      Stream lOutStream = pClientStream;
      StreamReader lClientStreamReader = new StreamReader(pClientStream);
      String lIMAP4Command = String.Empty;
      ...
      try
      {
        do
        {
          // Read IMAP4 command line
          lIMAP4Command = lClientStreamReader.ReadLine();

          if (Program.cDebuggingOn)
            Console.WriteLine(String.Format("DoIMAP4SProcessing(): Request command: \"{0}\"", lIMAP4Command));

          lFuncRetVal = ProcessRequest(ref lIMAP4Command, ref lUsername, ref lPassword);
           ...
           * Logout
           */
          if (!String.IsNullOrEmpty(lIMAP4Command) && lIMAP4Command.Trim().ToLower().StartsWith("logout"))
            break;
