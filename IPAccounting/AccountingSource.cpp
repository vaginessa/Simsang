...
//printf("SourceAccountingCallback  : (TCP)          %s:%d->%s:%d   %d\n", lSrcIP, lSrcPort, lDstIP, lDstPort, lTCPDataLen);
      ProcessSourceIPTraffic(lScanParams, "TCP", lIPHdr->saddr, lIPHdr->daddr, lSrcPort, lDstPort, lTCPDataLen);
...
//printf("SourceAccountingCallback  : (UDP)          %s:%d->%s:%d   %d\n", lSrcIP, lSrcPort, lDstIP, lDstPort, lTCPDataLen);
      ProcessSourceIPTraffic(lScanParams, "UDP", lIPHdr->saddr, lIPHdr->daddr, lSrcPort, lDstPort, lUDPDataLen);
...
//printf("SourceAccountingCallback  : (UDP)          %s:%d->%s:%d   %d\n", lSrcIP, lSrcPort, lDstIP, lDstPort, lTCPDataLen);
      ProcessSourceIPTraffic(lScanParams, "ICMP", lIPHdr->saddr, lIPHdr->daddr, lSrcPort, lDstPort, lICMPDataLen);
...
DWORD WINAPI SourceVisualisationThread(LPVOID lpParam)
{
  ...
  while (1)
  {
    ...
    {
      system("cls");
     	printf("%10s %12s %15s    %-20s    %s\n\n", "No. packets", "Data volume", "Last packet", "Source IP", "Host name");
